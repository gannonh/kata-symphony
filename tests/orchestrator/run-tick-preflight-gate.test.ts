...
describe('runTickPreflightGate', () => {
  it('blocks dispatch on preflight failure and awaits logFailure before returning', async () => {
    ...
    let resolveLogFailure!: () => void
    const logFailureDeferred = new Promise<void>((resolve) => {
      resolveLogFailure = resolve
    })
    const logFailure = vi.fn(async (failureErrors: DispatchPreflightError[]) => {
      expect(failureErrors).toEqual(errors)
      await logFailureDeferred
    })
