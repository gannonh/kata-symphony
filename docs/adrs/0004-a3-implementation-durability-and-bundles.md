...
7. **Pinned publication identity and bounded authentication.** Automatic intents capture forge owner, repository, base branch, and publication branch. Automatic implementation refreshes the trusted remote base branch before capturing the base SHA, so local commits that are absent from the publication remote cannot become thin-bundle prerequisites. Publication verifies that the captured base remains reachable from the pinned remote base before importing the result bundle; missing base history is a terminal publication conflict with no branch push. Recovery rejects active configuration drift and derives the HTTPS remote from that pinned identity. The trusted publisher supplies the GitHub token only through a subprocess-scoped HTTP authorization header; network Git calls disable prompting, time out, and redact credentials. A persisted draft-PR artifact is revalidated against live GitHub state through final-comment publication, and its author must match the authenticated publisher. The intent remains pending through final-comment publication; Symphony records completion events and the published run decision before terminalizing the intent, so restart recovery cannot strand a completed PR behind a pending run.

8. **Typed factory state projection.** Specification, implementation, and review lifecycle events feed an additive in-memory factory snapshot through the shared `SnapshotHandle`. The TUI renders typed factory sessions, bounded completions, and usage totals from that field; legacy worker snapshot fields retain their existing meaning.
...
- Both recovery commands reach the orchestrator over HTTP first and fall back to the durable store only when nothing answers. The store is guarded by an exclusive lock held for the orchestrator's lifetime, so a store-only command would fail exactly when an operator needs it — while Symphony runs. The commands call `GET /api/v1/publications/blocked` and `POST /api/v1/publications/{intent_id}/reset?operator=` at `server.host`/`server.port` (`--port` overrides), fall back on a connection failure, and when the store is *also* locked report that Symphony is running but unreachable at the configured address rather than surfacing a bare lock error. The two routes are mutating and unauthenticated, matching the existing `post_steer` / `post_refresh` / `delete_context_entry` surface; authenticated control is PRD slice B2.
- Direct live draft-PR, Agent Review preview, and Ratatui TUI UAT passed on `uat-symphony` Project #16 (issue #45, draft PR #46). Restart-during-publication recovery, full cleanup proof, and Docker execution remain operator residuals.
